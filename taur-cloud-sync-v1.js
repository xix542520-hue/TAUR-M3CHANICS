...
(()=>{
 ...
 let client=null,businessId=localStorage.getItem('TAUR_BUSINESS_ID')||'',timer=null,remoteReady=false,syncing=false,loadingRemote=false,channel=null,coreSyncTimer=null,coreSyncBound=false,coreSyncQueued=false,syncStage='idle';
...
async function init(){try{await loadSdk();client=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY);const {data:{session}}=await client.auth.getSession();if(session)await signedIn(session);else statusBar(false);client.auth.onAuthStateChange((event,session)=>setTimeout(()=>session?signedIn(session):statusBar(false),0));window.taurCloud={open:openPanel,signOut,sync:syncNow,migrate:migrateLocal,invite};}catch(e){console.error(e);toast('Cloud initialization failed')}}
 ...
 function setCloudPanelStatus(state,message){const text=message||state||'Checking connection…';window.taurCloudDiagnosticsState=state||'CONNECTING';const el=document.getElementById('taurCloudStatus');if(el)el.textContent=text;const body=document.querySelector('#taurCloudPanel .taur-cloud-user');if(body){const marker='STATUS: ';const i=body.innerHTML.indexOf(marker);if(i>=0){const j=body.innerHTML.indexOf('<',i+marker.length);const end=j>=0?j:body.innerHTML.length;body.innerHTML=body.innerHTML.slice(0,i+marker.length)+esc(text)+body.innerHTML.slice(end)}}}
 async function signedIn(session){remoteReady=false;setCloudPanelStatus('CONNECTING','Connecting to TAUR Cloud…');try{setCloudPanelStatus('CONNECTING','Authenticating…');const {data,error}=await client.rpc('bootstrap_taur_business',{business_name:'TAUR M3CHANICS'});if(error)throw new Error('Business bootstrap failed: '+error.message);if(!data)throw new Error('Business bootstrap returned no business ID');businessId=data;localStorage.setItem('TAUR_BUSINESS_ID',businessId);setCloudPanelStatus('CONNECTING','Loading cloud records…');await loadRemote();setCloudPanelStatus('CONNECTING','Starting realtime sync…');subscribe();remoteReady=true;bindCoreSync();setCloudPanelStatus('CONNECTING','Syncing this device…');await syncNow();statusBar(true);setCloudPanelStatus('CONNECTED','CONNECTED');toast('TAUR CLOUD connected',true)}catch(e){remoteReady=false;console.error('[TAUR CLOUD] signedIn failed',e);setCloudPanelStatus('ERROR','ERROR: '+(e.message||e));toast('Cloud connection error: '+(e.message||e))}}
 ...
 window.taurCloudDiagnostics=()=>({remoteReady,businessId:!!businessId,syncing,loadingRemote,syncStage,lastReconciliationPlan:lastReconciliationPlan?JSON.parse(JSON.stringify(lastReconciliationPlan)):null});
 async function syncNow(){if(!client||!businessId||syncing)return false;const local=localDb();if(!local)return false;syncing=true;syncStage='fetch-remote';try{
   ...
   remoteReady=true;statusBar(true,'SYNCED');syncStage='idle';return true;
 }catch(e){syncStage='error';console.error(e);toast('Cloud sync failed: '+(e.message||e));return false}finally{syncing=false;if(!coreSyncQueued&&syncStage!=='error')syncStage='idle';if(coreSyncQueued){coreSyncQueued=false;scheduleCoreSync()}}}
 ...
 async function renderPanelBody(){const body=document.getElementById('tcBody');if(!body)return;const {data:{session}}=await client.auth.getSession();if(!session){body.innerHTML='<div class="muted">Sign in to the TAUR business account.</div><label>EMAIL</label><input id="tcEmail" type="email" autocomplete="email"><label>PASSWORD</label><input id="tcPass" type="password" autocomplete="current-password"><button id="tcSignIn">SIGN IN</button><button class="secondary" id="tcSignUp">CREATE ACCOUNT</button><button class="secondary" id="tcReset">FORGOT PASSWORD / RESET</button>';tcSignIn.onclick=()=>auth(false);tcSignUp.onclick=()=>auth(true);tcReset.onclick=resetPassword;return}const {data:members}=await client.from('business_members').select('user_id,role,created_at').eq('business_id',businessId);const isAdmin=(members||[]).some(m=>m.user_id===session.user.id&&['owner','admin','partner'].includes(m.role));body.innerHTML='<div class="taur-cloud-user"><b>'+esc(session.user.email||'')+'</b><br>BUSINESS: TAUR M3CHANICS<br>STATUS: '+(remoteReady?'CONNECTED':(window.taurCloudDiagnosticsState||'CONNECTING'))+'</div><button id="tcSync">SYNC NOW</button><button class="secondary" id="tcMigrate">UPLOAD THIS DEVICE</button>'+(isAdmin?'<button class="secondary" id="tcInvite">INVITE / ADD PARTNER</button>':'')+'<button class="danger" id="tcOut">SIGN OUT</button>';tcSync.onclick=async()=>{if(await syncNow())toast('Cloud sync complete',true);renderPanelBody()};tcMigrate.onclick=migrateLocal;tcOut.onclick=signOut;if(isAdmin)tcInvite.onclick=invite}
 async function resetPassword(){const email=(document.getElementById('tcEmail')?.value||'').trim();if(!email)return alert('Enter your TAUR email first.');const {error}=await client.auth.resetPasswordForEmail(email,{redirectTo:window.location.href});if(error)return alert(error.message);alert('Password reset email sent. Check your inbox and follow the link to choose a new password.');}
