...
async function init(){try{await loadSdk();client=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY);const {data:{session}}=await client.auth.getSession();if(session)await signedIn(session);else statusBar(false);client.auth.onAuthStateChange((event,session)=>setTimeout(()=>session?signedIn(session):statusBar(false),0));window.taurCloud={open:openPanel,signOut,sync:syncNow,migrate:migrateLocal,invite};}catch(e){console.error(e);toast('Cloud initialization failed')}}
 ...
 function setCloudPanelStatus(state,message){const text=message||state||'Checking connection…';const el=document.getElementById('taurCloudStatus');if(el)el.textContent=text;const body=document.querySelector('#taurCloudPanel .taur-cloud-user');if(body){const marker='STATUS: ';const i=body.innerHTML.indexOf(marker);if(i>=0){const j=body.innerHTML.indexOf('<',i+marker.length);const end=j>=0?j:body.innerHTML.length;body.innerHTML=body.innerHTML.slice(0,i+marker.length)+esc(text)+body.innerHTML.slice(end)}}}
