...
(()=>{
 ...
 const paymentsFor=id=>window.TAUR?.payments?.forJob?.(id)||db.payments.filter(p=>p.jobId===id);
 ...
 function tipPayment(id){const j=job(id);if(!j)return;const quoted=Number(j.total||0),paid=typeof jobPaid==='function'?jobPaid(id):paymentsFor(id).reduce((n,p)=>n+Number(p.amount||0),0),tip=tipFor(id),balance=Math.max(0,quoted-(paid-tip));const w=modal('RECORD PAYMENT + TIP',`<div class="taur-dc-stats"><div><b>${moneyX(quoted)}</b><span>QUOTE</span></div><div><b>${moneyX(balance)}</b><span>QUOTE BALANCE</span></div><div><b>${moneyX(tip)}</b><span>TIPS TO DATE</span></div></div><div class="taur-dc-grid"><div><label>PAYMENT TOWARD QUOTE</label><input id="dcBase" type="number" min="0" step="0.01" value="${balance.toFixed(2)}"></div><div><label>TIP</label><input id="dcTip" type="number" min="0" step="0.01" value="0.00"></div><div class="taur-dc-full"><label>METHOD</label><select id="dcMethod"><option>CASH</option><option>CARD</option><option>VENMO</option><option>CASH APP</option><option>ZELLE</option><option>OTHER</option></select></div></div><div class="taur-dc-hint">Tip is tracked separately from the quoted job price.</div>`,`<button class="secondary" id="dcCancel">CANCEL</button><button id="dcSave">RECORD</button>`);w.querySelector('#dcCancel').onclick=close;w.querySelector('#dcSave').onclick=async()=>{const base=Number(w.querySelector('#dcBase').value||0),t=Number(w.querySelector('#dcTip').value||0);if(!Number.isFinite(base)||base<0||!Number.isFinite(t)||t<0||base+t<=0)return alert('Enter a payment and/or tip.');if(base>balance)return alert('Quote payment cannot exceed the remaining quote balance. Put the extra amount in TIP.');const paymentData={jobId:id,amount:base+t,baseAmount:base,tip:t,created:new Date().toISOString(),method:w.querySelector('#dcMethod').value};const payment=window.TAUR?.payments?.create?window.TAUR.payments.create(paymentData):null;if(!payment){db.payments.push({id:uid(),...paymentData});saveX();window.dispatchEvent(new Event('taur-cloud-sync'));}const cloudSync=window.taurCloud&&typeof window.taurCloud.sync==='function'?window.taurCloud.sync():null;if(cloudSync&&typeof cloudSync.then==='function')await cloudSync;close();if(typeof taurXipOpenJobFile==='function')taurXipOpenJobFile(id);else if(typeof render==='function')render()}}
 function editJob(id){
  const j=job(id);if(!j)return;
  const pricing=(()=>{try{const x=JSON.parse(localStorage.getItem('TAUR_PRICEBOOK_V1')||'[]');return Array.isArray(x)?x:[]}catch{return []}})();
  const currentPicks=Array.isArray(j.priceBookSelections)?j.priceBookSelections:(Array.isArray(j.addOns)?j.addOns:[]);
  const customerOptions=(db.customers||[]).map(c=>`<option value="${escX(c.id)}" ${c.id===j.customerId?'selected':''}>${escX(c.name||'Unnamed customer')}</option>`).join('');
  const vehicleOptions=()=>`<option value="">No vehicle</option>${(db.vehicles||[]).filter(v=>!wCustomer||v.customerId===wCustomer).map(v=>`<option value="${escX(v.id)}" ${v.id===j.vehicleId?'selected':''}>${escX([v.year,v.make,v.model].filter(Boolean).join(' ')||'Vehicle')}</option>`).join('')}`;
 ...
 function delJob(id){const j=job(id);if(!j)return;if(!confirm(`DELETE JOB?\n\n${j.title||'Untitled Job'}\n\nThis removes the job only. Linked payment/history records are preserved.`))return;if(window.TAUR?.jobs?.remove){const removed=window.TAUR.jobs.remove(id);if(!removed)return alert('This job has linked records. Remove or reassign those records first so TAUR does not orphan history.')}else{db.jobs=db.jobs.filter(x=>x.id!==id);saveX()}close();render()}
 function delCustomer(id){const c=customer(id);if(!c)return;const hasJobs=db.jobs.some(j=>j.customerId===id),hasVehicles=db.vehicles.some(v=>v.customerId===id),hasReferrals=db.referrals?.some(r=>r.customerId===id);if(hasJobs||hasVehicles||hasReferrals)return alert('This customer has linked vehicles, jobs, or referrals. Delete or reassign those records first.');if(!confirm(`DELETE CUSTOMER?\n\n${c.name||'Unnamed customer'}`))return;const removed=window.TAUR?.customers?.remove?window.TAUR.customers.remove(id):null;if(!removed){db.customers=db.customers.filter(x=>x.id!==id);saveX()}render()}
 function delVehicle(id){const v=vehicle(id);if(!v)return;const hasJobs=db.jobs.some(j=>j.vehicleId===id),hasReferrals=db.referrals?.some(r=>r.vehicleId===id);if(hasJobs||hasReferrals)return alert('This vehicle has linked jobs or referrals. Delete or reassign those records first.');if(!confirm(`DELETE VEHICLE?\n\n${v.year||''} ${v.make||''} ${v.model||''}`))return;const removed=window.TAUR?.vehicles?.remove?window.TAUR.vehicles.remove(id):null;if(!removed){db.vehicles=db.vehicles.filter(x=>x.id!==id);saveX()}render()}
