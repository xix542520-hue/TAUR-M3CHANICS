...
(()=>{
 ...
 function delJob(id){const j=job(id);if(!j)return;if(!confirm(`DELETE JOB?\n\n${j.title||'Untitled Job'}\n\nThis removes the job and its payment records from this device.`))return;const removed=window.TAUR?.jobs?.remove?window.TAUR.jobs.remove(id):null;if(!removed){db.jobs=db.jobs.filter(x=>x.id!==id);db.payments=db.payments.filter(p=>p.jobId!==id);saveX()}else{(window.TAUR?.payments?.forJob?.(id)||[]).slice().forEach(p=>window.TAUR.payments.remove(p.id));}close();render()}
