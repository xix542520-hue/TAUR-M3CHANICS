...
(()=>{
 ...
 function editJob(id){
  ...
  const customerOptions=(window.TAUR?.customers?.list?.()||db.customers||[]).map(c=>`<option value="${escX(c.id)}" ${c.id===j.customerId?'selected':''}>${escX(c.name||'Unnamed customer')}</option>`).join('');
 ...
 function delJob(id){const j=job(id);if(!j)return;if(!confirm(`DELETE JOB?\n\n${j.title||'Untitled Job'}\n\nThis removes the job only. Linked payment/history records are preserved.`))return;if(window.TAUR?.jobs?.remove){const removed=window.TAUR.jobs.remove(id);if(!removed)return alert('This job has linked records. Remove or reassign those records first so TAUR does not orphan history.')}else{if(Array.isArray(db.jobs))db.jobs.splice(db.jobs.findIndex(x=>x.id===id),1);saveX()}close();render()}
