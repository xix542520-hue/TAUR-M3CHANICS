...
(()=>{
 ...
 function editJob(id){
  ...
  const refillPackages=()=>{const t=typeEl.value, picks=Array.isArray(j.priceBookSelections)?j.priceBookSelections:[];packages.innerHTML=pricing.filter(x=>x.type===t).map(x=>`<label class="taur-dc-pick"><input type="checkbox" data-pick="${escX(x.id)}" ${picks.some(p=>p?.id===x.id)?'checked':''}><span><b>${escX(x.name)}</b><small>${moneyX(x.price)} • ${escX(x.description||'')}</small></span><strong>${moneyX(x.price)}</strong></label>`).join('')||'<div class="taur-dc-hint">No price-book services for this division.</div>';packages.querySelectorAll('[data-pick]').forEach(box=>box.onchange=()=>{const selected=[...packages.querySelectorAll('[data-pick]:checked')].map(b=>pricing.find(x=>x.id===b.dataset.pick)).filter(Boolean);j.__editPackageSelection=selected;const sum=selected.reduce((n,x)=>n+Number(x.price||0),0);if(selected.length){w.querySelector('#eTotal').value=sum.toFixed(2)}})};
  typeEl.onchange=()=>{j.__editPackageSelection=[];refillPackages();};refillPackages();
  ...
  w.querySelector('#dcSave').onclick=()=>{
    ...
    if(Array.isArray(selected)&&selected.length){j.priceBookSelections=selected.map(x=>({id:x.id,name:x.name,price:Number(x.price||0),type:x.type}));j.addOns=j.priceBookSelections.slice();}
